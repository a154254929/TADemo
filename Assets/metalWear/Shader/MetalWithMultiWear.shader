Shader "Unlit/MetalWithMultiWear"
{
    Properties
    {
        ...
        _RustTexture("Rust Texture", 2D) = "white"{}
        _RustLimit("Rust Limit", Range(0, 1)) = 0.2887
        _RustColor("Rust Color", Color) = (1.0, 1.0, 1.0, 1.0)
        _RustTextureContrast("Rust Texture Contrast", range(0, 2)) = 1
        _RustTextureSaturation("Rust Texture Saturation" , range(0, 2)) = 1
        _RustTextureBrightness("Rust Texture Brightness" , range(0, 2)) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float _RustLimit;
            fixed4 _RustColor;
            float _RustTextureContrast;
            float _RustTextureSaturation;
            float _RustTextureBrightness;
            ...
            float4 Overlay(float4 color1, float4 color2)
            {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                fixed4 RustCol = tex2D(_RustTexture, i.uv);
                ...
                //生锈纹理
                fixed4 avgColor = fixed4(1.0, 1.0, 1.0, 1.0);
			    //根据Contrast在对比度最低的图像和原图之间差值
			    RustCol = lerp(avgColor, RustCol, _RustTextureContrast);
                //饱和度
                fixed grey = max(max(RustCol.r, RustCol.g), RustCol.b);
                RustCol = lerp(fixed4(fixed3(1.0, 1.0, 1.0) * grey, 1.0), RustCol, _RustTextureSaturation);
                //明度
                RustCol = lerp(fixed4(0, 0, 0, 1.0), RustCol, _RustTextureBrightness);
                RustCol = Overlay(RustCol, _RustColor);
                //return basicColor;

                float RustValue = smoothstep(0, _RustLimit, i.col.r);
                basicColor = lerp(basicColor, RustCol, RustValue);
