Shader "Unlit/EnvMetalWithWearOrSpecilar"
{
    Properties
    {
        _EnvCube("Environment Cubemap", Cube) = "white"{}
        _EnvPower("Environment Power", Range(0, 1)) = 0.234
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                fixed4 Env = texCUBE(_EnvCube, reflect_dir);
                fixed4 basicColor = lerp(lerp(_EnvColor, Env, _EnvPower), _WearColor, wearValue);
                ...
                float SpecularStep = lerp(step(_SpecularLimit, pow(saturate(dot(reflect_dir, normal_world)), _Smooth)), _WearRoughness, wearValue);
